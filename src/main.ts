...
const btnSubmit = <HTMLButtonElement>document.getElementById('btnSubmit');
...
async function clickUpdateEmployee(employee: EmployeeType) {
	if (
		validateEmail(txtEmail.value) &&
		validateName(txtName.value) &&
		validatePassword(txtPassword.value) &&
		checkEmail(txtEmail.value, employee._id)
	) {
		...
		if (fileImage.files && fileImage.files.length > 0) {
			employee.image = (await blobToData(fileImage.files[0])) as string;
		} else employee.image = '';
...
txtPassword.onblur = () => {
	if (!validatePassword(txtPassword.value)) {
		showError(
			txtPassword,
			'Password must be at least 8 characters, at least one number and both lower and uppercase letters and special characters'
		);
	}
};
